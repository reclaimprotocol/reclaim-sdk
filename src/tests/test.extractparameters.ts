...
describe('Extract parameters', () => {
	it('should extract parameters', () => {
		...
		const parametersExtracted = reclaimprotocol.utils.extractParameterValuesFromRegex(selectionRegex, PROOFS_WITHOUT_XPATH)
	...
	it('should throw error for inconsistent regexes passed through the proof', () => {
		...
		expect(() => reclaimprotocol.utils.extractParameterValuesFromRegex(selectionRegex, PROOFS_WITHOUT_XPATH)).toThrowError('Invalid number of proofs')
	})
})
